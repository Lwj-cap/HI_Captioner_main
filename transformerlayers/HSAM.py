...
class ScaledDotProductAttention1(nn.Module):  ##点乘注意力
    def __init__(self, d_k, dropout=.1):
        ...
        # self.pe = PositionalEncoding(d_model=64, len_q=192)#train_modelv1
...
class ScaledDotProductAttention2(nn.Module):
    def __init__(self, d_k, dropout=0.1, total_layers=3):
        super(ScaledDotProductAttention2, self).__init__()
        self.d_k = d_k
        self.total_layers = total_layers
        self.pp = LayerScaledPositionalEncoding(d_model=64, total_layers=3, max_len=144)
        self.softmax = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(dropout)
        # Initialize a scale factor for each layer
        self.scale_factors = [np.sqrt(d_k) * (i + 1) / total_layers for i in range(total_layers)]

    def forward(self, q, k, v, attn_mask=None):
        batch_size, num_heads, seq_len, d_k = q.size()
        context_layers = []
        attn_layers = []
        pos = self.pp(q).permute(0,2,1,3)
        pos1 = self.pp(k).permute(0,2,1,3)
        pos2 = torch.matmul(pos, pos1.transpose(-1, -2)).to(device)

        for layer_idx in range(self.total_layers):
            # Use a unique scale factor per layer
            dynamic_scale = self.scale_factors[layer_idx]

            # Calculate attention scores with dynamic scaling
            scores = torch.matmul(q, k.transpose(-1, -2)) / dynamic_scale
            # scores = scores.permute(0,2,1,3)
            scores = scores + pos2
            # Apply attention mask if provided
            if attn_mask is not None:
                assert attn_mask.size() == scores.size(), "Attention mask shape does not match scores shape."
                scores = scores.masked_fill_(attn_mask, -1e9)  # large negative value for masked positions

            # Apply softmax to normalize the attention scores
            attn = self.softmax(scores)
            attn = self.dropout(attn)

            # Multiply the normalized attention scores with the value vector to get the output
            context = torch.matmul(attn, v)

            # Accumulate outputs from all layers
            context_layers.append(context)
            attn_layers.append(attn)

        # Sum the outputs from all layers
        context = torch.stack(context_layers).sum(dim=0)
        attn = torch.stack(attn_layers).sum(dim=0)

        return context, attn
class LayerScaledPositionalEncoding(nn.Module):
    def __init__(self, d_model, total_layers, max_len):
        super(LayerScaledPositionalEncoding, self).__init__()
        self.d_model = d_model
        self.max_len = max_len
        self.total_layers = total_layers

        # Create a positional encoding matrix that is long enough
        self.pe_enc = torch.zeros(max_len, d_model,device=device)
        position = torch.arange(0, max_len, dtype=torch.float,device=device).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2,device=device).float() * (-math.log(10000.0) / d_model))

        self.pe_enc[:, 0::2] = torch.sin(position * div_term)
        self.pe_enc[:, 1::2] = torch.cos(position * div_term)
        self.pe_enc = self.pe_enc.unsqueeze(0).transpose(0, 1)

        # Create scale factors for each layer
        self.scale_factors = [1 - (i / total_layers) for i in range(total_layers)]
        self.scale_factors = torch.tensor(self.scale_factors).unsqueeze(1).unsqueeze(2)

        # self.register_buffer('pe_enc', self.pe_enc)
    ...
    def forward(self, x):
        ...
        """
        # Get the scale factor for the current layer
        for layer_idx in range(self.total_layers):
            scale_factor = self.scale_factors[layer_idx].to(x.device)

        # Scale the positional encoding and add to input
        #     x = x + self.pe_enc[:x.size(0), :] * scale_factor
            x = (x.permute(0,2,1,3) + self.pe_enc[:, :self.max_len, :]* scale_factor).to(device)
            return x
