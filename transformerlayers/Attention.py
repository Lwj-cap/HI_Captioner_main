...
from transformerlayers.HSAM import Linear
from transformerlayers.HSAM import ScaledDotProductAttention,ScaledDotProductAttention1,ScaledDotProductAttention2
from transformerlayers.modules3 import LayerNormalization
...
class MultiHeadAttention2(nn.Module):
    def __init__(self, d_k, d_v, d_model, n_heads, dropout):
        ...
        self.n_heads = n_heads
        self.self_attn = _MultiHeadAttention(d_k, d_v, d_model, n_heads, dropout)
        self.cross_attn = _MultiHeadAttention(d_k, d_v, d_model, n_heads, dropout)
        self.proj = nn.Linear(n_heads * d_v, d_model)
        self.dropout = nn.Dropout(dropout)
        self.layer_norm = nn.LayerNorm(d_model)

    def forward(self, q, k, v, enc_outputs, self_attn_mask, enc_attn_mask):
        # Self-attention
        residual = q
        context, self_attn = self.self_attn(q, q, q, attn_mask=self_attn_mask)
        context = self.dropout(self.proj(context))
        context = self.layer_norm(residual + context)

        # Cross-attention
        residual = context
        context, cross_attn = self.cross_attn(context, enc_outputs, enc_outputs, attn_mask=enc_attn_mask)
        context = self.dropout(self.proj(context))
        context = self.layer_norm(residual + context)

        return context, self_attn, cross_attn
...
class _MultiHeadAttention1(nn.Module):
    def __init__(self, d_k, d_v, d_model, n_heads, dropout):
        ...
        self.self_attention = ScaledDotProductAttention2(d_k, dropout)
